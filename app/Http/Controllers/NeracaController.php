            ...
            foreach ($groupCodes as $key => $groupCode) 
            {
                ...
                foreach ($groupCode as $key1 => $code) 
                {
                    ...
                    else if($code->normal_balance_id == NORMAL_BALANCE_KREDIT)
                    {
                        ...
                        if($code->id = 123 || $code->id = 126)
                        {
                            $saldoKreditJurnalUmum = DB::table('t_jurnal')->where('akun_kredit', $code->CODE)->where('jurnalable_type', 'App\Models\JurnalUmum')->whereBetween('tgl_transaksi', [$startComparePeriod, $endComparePeriod])->sum('kredit');
