...
<div class="card">
    <div class="card-body">
        <form action="{{ route('jurnal-umum-edit', ['id' => $jurnalUmum->id]) }}" method="POST">
            @csrf
                ...
                <div class="row" id="form1">
                    <div class="col-md-6 form-group">
                        ...
                        @foreach ($codes as $code)
                            <option value="{{ $code->id }}" @if($code->id == $jurnalUmum->code_id) selected @endif>{{ $code->CODE }}</option>
                        @endforeach
                        </select>
                    </div>
                    <div class="col-md-6 form-group">
                        <label for="nominal1">Besar Nominal</label>
                    <input type="text" name="nominal" id="nominal" value="{{ $jurnalUmum->nominal }}" onkeypress="return isNumberKey(event)" data-form="1" class="form-control nominal" placeholder="Besar Nominal" autocomplete="off" required >
