                ...
                <div class="row">
                    <div class="col-md-8">
                        <div class="row">
                            ...
                            <div class="col-md-6 form-group">
                                ...
                                <input type="text" name="role_name"
                                       ...
                                       placeholder="Your Role" readonly class="form-control">
                                <input type="hidden" name="role_id"
                                       value="{{ ($user->roles->first())? $user->roles->first()->id:'' }}">
                            </div>
                            <div class="col-md-6 form-group">
                                <label>Name</label>
                                ...
                                <div class="col-md-6 form-group">
                                    ...
                                    @if (\Auth::user()->isAnggota())
                                        @if ($anggota && $anggota->kelas_company_id)
                                            <input type="text" name="kelas_company_name" class="form-control"
                                                   value="{{ $anggota->kelasCompany->nama }}" readonly>
                                            <input type="hidden" name="kelas_company" class="form-control"
                                                   value="{{ $anggota->kelasCompany->id }}" readonly>
                                        @else
                                            <select name="kelas_company" class="form-control">
                                                <option value="">Choose One</option>
                                                @foreach($classList as $itemClass)
                                                    <option
                                                        value="{{ $itemClass->id }}" {{ ($anggota && $anggota->kelas_company_id == $itemClass->id)? 'selected':'' }}>{{ $itemClass->nama }}</option>
                                                @endforeach
                                            </select>
                                        @endif
                                    @else
                                        <select name="kelas_company" class="form-control">
                                            ...
                                            @foreach($classList as $itemClass)
                                                <option
                                                    value="{{ $itemClass->id }}" {{ ($penghasilan && $penghasilan->kelas_company_id == $itemClass->id)? 'selected':'' }}>{{ $itemClass->nama }}</option>
                                            @endforeach
                                        </select>
                                    @endif
                                </div>
                            ...
                            @if (\Auth::user()->isAnggota())
                                @foreach ($listJenisPenghasilan as $jenisPenghasilan)
                                    <div class="col-md-6 form-group">
                                        ...
                                        <input type="text" id="penghasilanTertentu{{ $jenisPenghasilan->id }}"
                                               name="penghasilan[{{ $jenisPenghasilan->id }}]"
                                               class="form-control toRupiah" placeholder="{{ $jenisPenghasilan->name }}"
                                               @if ($listPenghasilan)
                                    ...
                                    <div class="col-md-6 form-group">
                                        ...
                                        <div class="custom-file">
                                            <input type="file" class="custom-file-input"
                                                   id="file_penghasilanTertentu{{ $jenisPenghasilan->id }}"
                                                   name="file_penghasilan[{{ $jenisPenghasilan->id }}]"
                                            ...
                                            @if($listPenghasilan && $listPenghasilan->where('id_jenis_penghasilan', $jenisPenghasilan->id)->first()->file_path)
                                                <label class="custom-file-label"
                                                       for="customFile">{{ $listPenghasilan->where('id_jenis_penghasilan', $jenisPenghasilan->id)->first()->file_path }}</label>
                                            @else
                        ...
                        <div class="form-group">
                            <button type="submit" class="form-control btn btn-sm btn-success"><i class="fa fa-save"></i>
                                Update
                            </button>
                        </div>
                    </div>
                        ...
                        <div class="form-group text-center">
                            ...
                            <div class="form-group">
                                <div class="col-md-12 text-center" id="photoKtpButton">
                                    ...
                                    @else
                                        <img class="img-fit" id="ktpPreview"
                                             src="{{ asset('img/no_image_available.jpeg') }}">
                                    @endif
                                    <span class="btn btn-default btn-file mt-2">
