...
@section('content')
<form action="{{ route('jenis-pinjaman-edit', ['id' => $pinjaman->kode_jenis_pinjam]) }}" method="post">
    {{csrf_field()}}
    <div class="card">
        <div class="card-body">
            <div class="row">
                <div class="col-md-12 mb-2">
                    <label>Tanggal Entri :  {{ date('d M Y', strtotime($pinjaman->tgl_entri)) }} </label>
                </div>
                <div class="form-group col-md-6">
                    <label>Kode Jenis Pinjaman</label>
                    <input type="text" class="form-control kode-jenis-pinjaman" name="kode_jenis_pinjam" size="54" placeholder="Kode Jenis Pinjaman" value="{{$pinjaman->kode_jenis_pinjam}}" readonly />
                </div>
                <div class="form-group col-md-6">
                    ...
                    <select name="tipe_jenis_pinjaman_id" class="form-control" required id="tipeJenisPinjamanOption">
						...
						@foreach ($tipe_jenis_pinjaman as $type)
                            <option value="{{ $type->id }}"
                                {{ $type->id == $pinjaman->tipe_jenis_pinjaman_id ? 'selected' : '' }}>
                                {{ $type->name }}
                            </option>
						@endforeach
					</select>
                ...
                <div class="form-group col-md-6">
                    ...
                    <input type="text" class="form-control" name="nama_pinjaman" size="54" placeholder="Jenis Pinjaman" value="{{ $pinjaman->nama_pinjaman }}"/>
                </div>
                <div class="form-group col-md-6">
                    <label>Lama Angsuran </label>
                    <input type="number" class="form-control" name="lama_angsuran" placeholder="Lama Angsuran" value={{ $pinjaman->lama_angsuran }} />
                </div>
                <div class="form-group col-md-6">
                    <label>Maksimal Pinjaman</label>
                    <input type="number" step="any" class="form-control" name="maks_pinjam" size="54" placeholder="Maksimal Pinjaman" value={{ $pinjaman->maks_pinjam }} />
                </div>
                {{-- <div class="form-group col-md-6">
                    <label>Bunga</label>
                    <input type="number" step="any" step="any" class="form-control" name="bunga" size="54" placeholder="Bunga" value={{ $pinjaman->bunga }} />
                </div> --}}
                <div class="form-group col-md-6">
                    <label>Asuransi</label>
                    <input type="number" step="any" step="any" class="form-control" name="asuransi" placeholder="Asuransi" value={{ $pinjaman->asuransi }} />
                </div>
                <div class="form-group col-md-6">
                ...
                <div class="col-md-6">
                    <button type="submit" class="btn btn-success"><i class="fa fa-save"></i> Update Jenis Pinjaman</button>
                </div>
            </div>
        </div>
    </div>
