...
<div class="row">
	...
	<div class="col-6">
		<ol class="breadcrumb float-sm-right">
			...
			<li class="breadcrumb-item active">Kartu Pinjaman</li>
		</ol>
	</div>
</div>
@endsection
					...
					<div class="col-md-12">
                        ...
                        @endforeach
							...
							<div class="col-12 form-group mt-2 " id="anggotaForm">
								<label for="detailAnggota">Preview Kartu Pinjaman</label>
								<div id="exportButton" class="d-none text-right my-2">
									@if(\Auth::user()->isAnggota())
										<a class="btn btn-sm btn-info" id="pdfButton"><i class="fas fa-download"></i> Export PDF</a>
									@else
	...
	<script>
		$(document).ready(function ()
		{
			@if ($request->kode_anggota)
				...
				$.get(baseURL + "/pinjaman/card/view/" + selectedValue, function( data ) {
					$('#detailAnggota').html(data);
				});
				var urlExport = baseURL + "/pinjaman/card/download";
		...
		$('#pilihyear').on('change', function ()
		{
			...
			var baseURL = {!! json_encode(url('/')) !!};
			$.get(baseURL + "/pinjaman/card/view/" + selectedValue+"?year="+selectedyear, function( data ) {
				$('#detailAnggota').html(data);
			});
            var urlExport = baseURL + "/pinjaman/card/download";
            $('#exportButton').removeClass('d-none');
