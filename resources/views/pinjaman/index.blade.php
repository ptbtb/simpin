                ...
                <div class="row">
                    @if ($role->id !== ROLE_ANGGOTA)
                        <div class="col-md-4 form-group">
                            ...
                            {!! Form::select('jenistrans', ['S' => 'Semua', 'A' => 'Saldo Awal', 'T' => 'Transaksi'], $request->jenistrans, ['id' => 'jenistrans', 'class' => 'form-control']) !!}
                        </div>
                    @endif
                    <div class="col-md-4 form-group">
                    ...
                    <div class="col-md-4 form-group">
                        <label>From</label>
                        <input id="from" type="text" name="from" class="form-control datepicker" placeholder="yyyy-mm-dd"
                            value="{{ $request->from ? $request->from : '' }}">
                    </div>
                    <div class="col-md-4 form-group">
                        <label>To</label>
                        <input id="to" type="text" name="to" class="form-control datepicker" placeholder="yyyy-mm-dd"
                    ...
                    @foreach ($listPinjaman as $pinjaman)
                        {{-- @if ($pinjaman->listAngsuran->count('*') > 0) --}}
                            <tr>
                                ...
                                @endif
                                <td>{{ \Carbon\Carbon::createFromFormat('Y-m-d', $pinjaman->tgl_transaksi)->format('d M Y') }}
                                </td>
                                <td>
                                ...
                                @if ($request->jenistrans == 'A')
                                    <th>Rp. {{ number_format($pinjaman->saldo_mutasi, 0, ',', '.') }}
                                        @can('edit saldo awal pinjaman')
                                            <button type="button" class="btn btn-danger openedit" data-toggle="modal"
                                                data-target="#modal-default" data-id="{{ $pinjaman->kode_pinjam }}"
                                                data-nama="{{ $pinjaman->anggota ? $pinjaman->anggota->nama_anggota : '-' }}"
                                                data-kode="{{ $pinjaman->anggota ? $pinjaman->anggota->kode_anggota : 0 }}"
                                                data-jumlah="{{ $pinjaman->saldo_mutasi }}">
                                ...
                                <td>
                                    <a href="{{ route('pinjaman-detail', ['id' => $pinjaman->kode_pinjam]) }}"
                                        class="btn btn-sm btn-info text-white" data-action='detail'><i
                                            class="fa fa-eye"></i> Detail</a>
                                    @can('edit pinjaman')
                                        <a class="btn btn-sm btn-warning text-white btn-edit" style="cursor: pointer"
                                            href="{{ route('pinjaman-edit', ['id' => $pinjaman->kode_pinjam]) }}"><i
                                                class="fa fa-edit"></i> Edit</a>
                                    @endcan
                                    @can('delete pinjaman')
                                        <a class="btn btn-sm btn-danger text-white btn-delete" style="cursor: pointer"
                                            data-action='delete' data-id='{{ $pinjaman->kode_pinjam }}'
                                            data-token='{{ csrf_token() }}'><i class="fa fa-trash"></i> Delete</a>
                                    @endcan
